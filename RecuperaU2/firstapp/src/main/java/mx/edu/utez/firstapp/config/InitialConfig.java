...
public class InitialConfig implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
       ...
       getOrSaveRole(new Role("ADMIN_ROLE"));
        getOrSaveRole(new Role("CLIENT_ROLE"));
        //Crear un usuario para que puedan iniciar sesión (person, user, user_role)
         person = getOrSavePerson(
                new Person("Angel", "moreno", null,
                        LocalDate.of(1998, 1, 19), "MOVM980519HM")
        );
         user = getOrSaveUser(
                new User("user", encoder.encode("user"), person)
        );
        saveUserRoles(user.getId(), userRole.getId());
    }
    @Transactional
